...
export function getRequests(req, res) {
  ...
  dbconnect.query(text, values, (err, result) => {
    if (result.rows[0].driver !== req.user.username) {
      res.status(403).send({
...
export function addRequest(req, res) {
  ...
  dbconnect.query(text, values, (err, result) => {
    ...
    } else if (req.user.username === result.rows[0].driver) {
      res.status(403).send({
...
export function respondToRequest(req, res) {
  ...
  dbconnect.query(text, values, (err, result) => {
    ...
    } else if (result.rows[0].driver !== req.user.username) {
      res.status(403).send({
...
export function getUserRequests(req, res) {
  const text = 'SELECT * FROM requests WHERE passenger = $1';
